...
test("desktop relay exposes the measured wait limit in tool help without changing validation", () => {
  const adapted = desktopRelayTool(MESSAGE_BOX_TOOL);
  assert.match(adapted.description ?? "", /wait_seconds: 45/u);
  ...
  assert.match(JSON.stringify(adapted.inputSchema), /45/u);
...
test("desktop stdio preserves the foreground request and explicit receipt", {
  ...
  assert.match(client.getInstructions() ?? "", /wait_seconds: 45/u);
  ...
  assert.match(tool?.description ?? "", /wait_seconds: 45/u);
  const input = {
    type: "request_action",
    request_id: requestId,
    wait_seconds: 37,
