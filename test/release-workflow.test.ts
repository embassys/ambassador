...
test("main publishes the approved Ambassador 0.2.6 release through npm OIDC", async () => {
...
test("every supported-agent guide uses the 0.2.6 package and current registration flow", async () => {
  ...
  for (const guide of guides) {
    ...
    assert.match(contents, /npx --yes @embassys\/ambassador@0\.2\.6 start/u, guide);
    assert.match(contents, /--local-token-env=AMBASSADOR_LOCAL_TOKEN/u, guide);
