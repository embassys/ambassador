...
const SECRET_MARKERS = [CENTRAL_JWT, OTHER_CENTRAL_JWT, CREDENTIAL_SCOPE, OTHER_CREDENTIAL_SCOPE];
...
function credentialStore(
  ...
  return new EncryptedFileCredentialStore(path, keyPath, scope, options);
}
...
test("writes only the strict cryptographic envelope and no plaintext", async (t) => {
  ...
  const stateKey = await readFile(item.keyPath);
  assert.equal(stateKey.length, 24);
  assert.equal((await readFile(item.path)).includes(stateKey), false);
...
test("a different internal state key cannot decrypt the credential", async (t) => {
  const item = await fixture(t);
  await credentialStore(item.path, item.keyPath).save(CENTRAL_JWT);
  const otherKeyPath = join(item.directory, "other.key");
  await writeFile(otherKeyPath, Buffer.alloc(24, 0xab), { mode: 0o600 });

  const wrongStore = credentialStore(item.path, otherKeyPath);
  await expectSafeRejection(() => wrongStore.load());
  assert.ok((await credentialStore(item.path, item.keyPath).load()) === CENTRAL_JWT);
});
...
test("a different central endpoint scope cannot decrypt the credential", async (t) => {
  ...
  await expectSafeRejection(() =>
    credentialStore(item.path, item.keyPath, OTHER_CREDENTIAL_SCOPE).load(),
  );
  assert.equal(await credentialStore(item.path, item.keyPath).load(), CENTRAL_JWT);
});
...
test("rejects authenticated-ciphertext tampering", async (t) => {
  ...
  await expectSafeRejection(() => credentialStore(item.path, item.keyPath).load());
});
...
test("never overwrites the first stored identity", async (t) => {
  ...
  await expectSafeRejection(() => credentialStore(item.path, item.keyPath).save(OTHER_CENTRAL_JWT));
...
test("serializes concurrent saves for the same identity path", async (t) => {
  const item = await fixture(t);
  const first = credentialStore(item.path, item.keyPath);
  const second = credentialStore(item.path, item.keyPath);
...
test("fails closed when injected Windows DACL enforcement fails", async (t) => {
  ...
  const store = credentialStore(item.path, item.keyPath, CREDENTIAL_SCOPE, {
    platform: "win32",
...
test("fails closed when an existing credential has no valid internal state key", async (t) => {
  const item = await fixture(t, "a2a-credential-key-format-test-");
  await credentialStore(item.path, item.keyPath).save(CENTRAL_JWT);
  await rm(item.keyPath);
  await expectSafeRejection(() => credentialStore(item.path, item.keyPath).load());
  await writeFile(item.keyPath, "not-a-key", { mode: 0o600 });
  await expectSafeRejection(() => credentialStore(item.path, item.keyPath).load());
});
