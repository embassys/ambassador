...
The service runs `a2a run --config <absolute path>`. Installation writes a definition but does not embed credentials. Environment-referenced credentials come from the user's service environment until OS credential vault support replaces them.
...
`a2a service install` writes or replaces only the sidecar's own definition. Start, stop, restart, status, and uninstall call the native manager without a shell. Commands pass argument arrays to child processes, so paths and binding IDs cannot become shell syntax.
