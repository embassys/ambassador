...
- SQLite journal with atomic ingestion, durable acknowledgements and reports, queue capacity, atomic claims, crash recovery, conservative clock-offset handling, private file permissions, and terminal states.
...
- 127 automated tests using local HTTP servers, real temporary SQLite files, and concurrent subprocess lock contention.
