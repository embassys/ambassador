...
- [ ] I02: replace the central fixtures, gateway contract tests, red
  inventories, and CI expectations with the current unversioned REST and DPoP
  contract.
- [ ] I03: implement email-only REST enrollment, body JWK binding, the current
  30-day token claims, `Authorization: Bearer`, optional nonce handling, and
  one current encrypted credential format.
- [ ] I03: delete bearer-only central credentials, central MCP transport,
  token arguments, REST-to-MCP fallback, reissue/recovery, migration, and the
  verbose transcript.
- [ ] I04: implement fixed REST-backed action, permission, poll, permission
  list, and acknowledgement tools.
- [ ] I04: delete activation, lease, conversation, reply, completion, and
  outcome clients and their fixture-only production wiring.
- [ ] I05: run two disposable Mailosaur identities through registration,
...
- [ ] Keep Mailosaur secrets in the macOS Keychain service
