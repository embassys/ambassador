...
export const DELIVERY_WORKFLOW_INSTRUCTIONS = [
  ...
  "If you need any owner input or confirmation, including confirmation about repeated similar requests, call message_box with type ask_owner, a new UUID request_id, the exact call_id, question, and input_type text or buttons before ending your turn. For buttons include the exact labels and values. Recording a question emails the owner and leaves the call pending. Text in this background transcript alone does not reach the owner. A distinct call_id is a separate request; do not reuse an earlier result submission or assume that a similar payload is a replay.",
