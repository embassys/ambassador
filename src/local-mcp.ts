...
const DISCOVERY_INSTRUCTIONS =
  "Embassys is available through this Ambassador MCP integration. For Embassys registration use register_agent with email and a user-chosen username; no website URL is needed. Complete the emailed code with verify_email. To contact another person's agent, use list_action_types and message_box. Check current enrollment before registering again.";
const SERVER_INSTRUCTIONS =
  "Embassys Ambassador connects this local agent to the Embassys agent network. A registered enrollment with an empty permissions list is still registered; no permissions have been granted yet. Do not register again or ask the user to reconfirm registration in that case. Use the verified enrollment email for this identity unless the user supplies a different address. For meetings with the user, include that email as an attendee when asking another agent to create the event; target_email selects the other agent and does not add the requester as an attendee. When not enrolled and the user asks to register or connect and gives an email, call register_agent immediately. After the emailed code arrives, call verify_email. Use message_box for actions, pending work, owner questions and results. A missing signup username is a setup question; ask the user to choose 5–32 letters or numbers. For targets, existing short usernames also work. get_available_actions reads accepted request types. set_available_actions replaces your full list only when your owner explicitly requests it; do not set it as an automatic setup step. Catalog verified describes review status, not a permission grant or a guarantee of execution. request_action waits up to ten minutes; on timeout show its continuation and check again when the user asks. Omit wait_seconds for the default 600-second wait. Use a shorter wait only when the user asks; pending work or an earlier host timeout is not a reason. If the host disconnects sooner, the user can ask to check the same saved request later. If an action requires a reason, use the user's stated purpose or a neutral restatement such as Requested by the user to obtain this contact's phone number. A neutral restatement is sufficient unless the action schema requires a more specific purpose; never invent that purpose. The target person's human decides permissions; the caller cannot approve a denial on their behalf. Present the actual result data, then acknowledge its receipt. A successful background turn does not mean the user received the answer." +
  "\n" +
...
function toolErrorMessage(code: string): string {
  switch (code) {
    ...
    case "username_unavailable":
      return "That public username is unavailable. Ask the user to choose another; do not restart registration for a different email.";
    case "username_invalid":
      return "Ask the user for a username with 5–32 letters or numbers, without spaces or punctuation.";
    case "action_not_accepted":
      return "The other agent does not accept new permission requests for this action. Use message_box with type get_available_actions to inspect its list. Do not substitute a different action or keep re-requesting.";
    ...
    case "credential_expired":
      return "Ambassador's central credential has expired. Saved inbox and session reads remain available. Keep local state. Open Embassys, sign in and use Account > Set up this device to restore execution for the existing identity. Do not register another identity or repeat uncertain requests.";
