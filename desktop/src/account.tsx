...
export function Account({
  ...
  useEffect(() => {
    ...
    setError("");
    setLoading(false);
    if (owner.status !== "signed_in") return;
    setLoading(true);
  ...
  return (
    <div className="account-page">
      <section className="settings-section">
        <div className="settings-row">
          <div>
            ...
            <p className="break">
              {owner.status === "signed_in"
                ? owner.email
                : "Use the email registered with your Embassys agent."}
            </p>
          </div>
          {owner.status === "signed_in" && (
            <button
        ...
        <p className="body-note">
          Your account is shared across app instances. Signing out keeps local servers running.
        </p>
        {owner.issue && !busy && (
          ...
          >
            {owner.status === "code_sent" ? (
              ...
              <>
                ...
                <p className="body-note">
                  First time here? Register a new local agent in Registration. Signing in here
                  doesn't restore a lost agent identity.
                </p>
              </>
            )}
          </form>
      ...
      {owner.status === "signed_in" && (
        <>
          <fieldset className="appearance-control account-tabs">
            <legend className="sr-only">Account view</legend>
            {(
              [
                ["requests", "Requests"],
