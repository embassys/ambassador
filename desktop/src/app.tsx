...
const pages: { id: Page; label: string; description: string }[] = [
  { id: "people", label: "People", description: "A place for the people you connect with." },
  {
    id: "account",
    label: "Settings",
    description: "Your requests, permissions and central message history.",
  },
  { id: "attention", label: "Inbox", description: "Requests and questions that need you." },
  {
    id: "registration",
  ...
  { id: "agents", label: "Agents", description: "Connect the tools you already use." },
  {
    id: "diagnostics",
    label: "Logs",
    description: "Activity and diagnostics from this device.",
  },
  {
    id: "settings",
...
function App() {
  ...
  const historyRead = useRef<(() => void) | undefined>(undefined);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: Navigation resets scroll; periodic status updates must not.
  useEffect(() => {
    pageBody.current?.scrollTo(0, 0);
  }, [page, id, dataSource, inboxRequest]);
  ...
  const firstInboxRequest = inboxEntries(workspace.loaded ? workspace.inbox : undefined)[0]?.key;
  useEffect(() => {
    if (
      onboarded &&
      page === "attention" &&
      ...
      setInboxRequest(firstInboxRequest);
  }, [onboarded, page, dataSource, inboxRequest, firstInboxRequest]);
  // biome-ignore lint/correctness/useExhaustiveDependencies: Account and installation changes must discard selected history.
  useEffect(() => {
    viewGeneration.current++;
    ...
    setInboxRequest("");
    setBusy(false);
  }, [id, snapshot?.owner.context, onboarded]);

  const service = serviceStatus(selected?.runtime);
  const lastSettingsRequest = useRef(localStorage.getItem("ambassador.settingsRequest") ?? "");
  useEffect(() => {
    if (!snapshot?.settingsRequest || snapshot.settingsRequest === lastSettingsRequest.current)
      return;
    lastSettingsRequest.current = snapshot.settingsRequest;
    localStorage.setItem("ambassador.settingsRequest", snapshot.settingsRequest);
    setPage(onboarded ? "account" : "settings");
    setDataSource("account");
    setSetupSettings(!onboarded);
  }, [snapshot?.settingsRequest, onboarded]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    root.dataset.focused = String(snapshot.focused);
    root.style.setProperty("--system-accent", snapshot.palette.accent);
    root.style.setProperty("--system-accent-text", snapshot.palette.accentText);
    root.style.setProperty("--system-link", snapshot.palette.link);
  ...
  useEffect(() => {
    ...
    setLogTo("");
    setError("");
    if ((!onboarded && !setupSettings) || !id || !["diagnostics", "agents"].includes(page)) return;
    setLoading(true);
    const command: DesktopCommand =
      page === "diagnostics" ? { type: "logs", instanceId: id } : { type: "setup", instanceId: id };
    const reader = createViewReader({
      read: () => call(command),
      publish: (result) => {
        if (page === "diagnostics") setLogs(result as DiagnosticPage);
  ...
  async function loadHistory(sessionId: string, before = Number.MAX_SAFE_INTEGER, quiet = false) {
    if (!id) return;
    if (quiet && historyReading.current) return;
    const generation = quiet ? viewGeneration.current : ++viewGeneration.current;
    const earlier = before !== Number.MAX_SAFE_INTEGER;
    ...
    try {
      ...
      if (viewGeneration.current === generation) {
        if (earlier) setHistoryExpanded(true);
        setHistory((previous) => {
          if ((!earlier && !quiet) || previous?.source !== "archive" || result.source !== "archive")
            return result;
          return mergeChatPages(previous, result, earlier ? "earlier" : "refresh");
        });
      }
    } catch {
      if (viewGeneration.current === generation)
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (!document.hidden) historyRead.current?.();
    }, 15000);
    return () => clearInterval(timer);
  }, [page, dataSource, historySession, historyExpanded, onboarded, historyAtCapacity]);
  async function deleteHistory() {
    ...
    try {
      ...
      if (result.deleted) workspace.clearPreview(historySession);
  ...
  const reviewSheet = snapshot?.review ? (
    <ReviewSheet
      key={snapshot.review.id}
      review={snapshot.review}
      answer={async (reviewId, choice) => {
        const result = (await call({ type: "review_answer", reviewId, choice })) as {
          accepted: boolean;
        };
        await refresh();
        if (!result.accepted) throw new Error("This review has expired.");
      }}
    />
  ) : null;
  ...
  if (!onboarded && !setupSettings)
    return (
      <div className="onboarding-with-status">
        <Onboarding
      ...
      <section className="settings-section">
        <div className="settings-row">
          ...
          <input
            type="checkbox"
            role="switch"
            aria-checked={snapshot.notifications.enabled}
            className="native-switch"
            aria-label="Notify me about requests and results"
            title="System settings and Do Not Disturb may silence alerts."
            checked={snapshot.notifications.enabled}
            disabled={busy || !snapshot.notifications.supported}
            onChange={(event) =>
              void mutate({ type: "set_notifications", enabled: event.target.checked })
            }
          />
        </div>
      </section>
      <section className="settings-section">
        <div className="settings-row">
          ...
          {snapshot.platform === "darwin" ? (
            <fieldset className="appearance-control" disabled={busy && !savingAppearance.current}>
  ...
  return (
    <div className={`app-shell ${onboarded ? "with-conversation-sidebar" : ""}`}>
      {reviewSheet}
      {onboarded && (
          ...
          <Navigation
            page={
              dataSource === "account" && page === "conversations"
                ? "network"
                : dataSource === "local" && page === "attention"
                  ? "activity"
                  : page
            }
            sessions={sessions}
            ...
            attention={workspace.bySession}
            inboxCount={workspace.loaded ? workspace.inboxCount : undefined}
            requests={workspace.loaded ? workspace.inbox : undefined}
            selectedRequest={inboxRequest}
            selectRequest={(key) => {
              navigate("attention");
              setInboxRequest(key);
            }}
            error={workspace.sessionError}
      ...
      <main>
        ...
        <div className={`page-body page-${page}`} ref={pageBody}>
          ...
          {onboarded &&
            page === "attention" &&
            dataSource === "account" &&
            selected?.runtime.state === "stopped" && (
              <div className="paused-notice" role="status">
                <span>Embassys is paused on this device.</span>
                <button
                  className="secondary"
                  type="button"
                  disabled={busy}
                  onClick={() => void mutate({ type: "start", instanceId: selected.id })}
                >
                  Resume
                </button>
              </div>
          ...
          {!snapshot ? (
            ...
            <>
              ...
              {dataSource === "account" &&
                ...
                (page !== "attention" || Boolean(inboxRequest)) && (
                  <Account
                    key={`${page}:${snapshot.owner.context}:${id}:${inboxRequest}`}
                    section={
                      page === "attention"
                        ? "requests"
                        : page === "permissions"
                    ...
                    call={call}
                    changed={refresh}
                    focusedRequest={page === "attention"}
                    requestSource={
                      page === "attention"
                        ? {
                            ...workspace.source,
                            data: workspace.loaded
                              ? selectInboxRequest(workspace.inbox, inboxRequest)
                              : undefined,
                          }
                        : undefined
                    }
              ...
              {page === "account" && snapshot && (
                <div className="preferences">
                  {selected && (
                    <section className="settings-section service-settings">
                      <div className="settings-row">
                        <div>
                          <strong>Background service</strong>
                          <p>
                            <span
                              className={`service-indicator ${service.tone}`}
                              aria-hidden="true"
                            />{" "}
                            {service.label} · {selected.name}
                          </p>
                        </div>
                        <button
                          type="button"
                      ...
                      <p className="body-note">
                        {running
                          ...
                          : "Saved work stays on this device while paused."}
                      </p>
                      <button
                        type="button"
                        className="text-button"
                  ...
                  <section className="device-links simple-settings-links">
                    <details className="advanced-settings">
                      ...
                      {(
                        [
                          ["settings", "Server controls", "account"],
                          ["registration", "Agent registration", "account"],
                          ["attention", "Local work", "local"],
                          ["permissions", "Local permissions", "local"],
                          ["conversations", "Network events", "account"],
                          ["diagnostics", "Logs", "account"],
                        ] as const
                      ).map(([destination, label, source]) => (
                        <button
                          type="button"
                      ...
                      <button
                        ...
                        className="device-link"
                        disabled={busy}
                        onClick={() => void mutate({ type: "owner_open_web" })}
                      >
                      ...
                      <button
                        ...
                        className="device-link"
                        disabled={busy}
                        onClick={() => void mutate({ type: "owner_reveal_logs" })}
                      >
              ...
              {page === "agents" && (
                <>
                  <p className="body-note">
                    Choose an agent to connect. Reopen existing chats after setup.
                  </p>
                  {loading ? (
              ...
              {page === "conversations" && dataSource === "local" && (
                ...
                >
                  ...
                  {historySession && (
                    <section className="chat-pending" aria-label="Conversation requests">
                      <Account
                        key={`${snapshot.owner.context}:${id}:${historySession}`}
                        section="requests"
                        snapshot={snapshot.owner}
                        ...
                        requestSource={{
                          ...workspace.source,
                          data: workspace.loaded
                            ? workspace.bySession.get(historySession)
                            : undefined,
                        }}
                      />
                    </section>
              ...
              {page === "diagnostics" && (
                <>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          void call({ type: "clear_logs", instanceId: id })
                            ...
                            .catch(() =>
                              setError(
                                "Logs could not be cleared. Check the server and try again.",
                              ),
                            )
                  ...
                  <section className="settings-section">
                    ...
                    <p className="body-note">
                      Exports use your current search and time range, up to 32 MiB per export.
                      Narrow the dates for larger selections. Nothing is uploaded.
                    </p>
                    ...
                    <p className="body-note">
                      Credentials are always removed. Bodies can contain personal messages and
                      action results.
                    </p>
                    <button
                      type="button"
                    ...
                    {exportPreview && (
                      <div className="export-preview" role="status">
                        <strong>
                          {exportPreview.recordCount} events ·{" "}
                          {Math.ceil(exportPreview.bytes / 1024)} KiB
                        </strong>
                        <p>
              ...
              {page === "settings" && (
                <>
                  {snapshot.instances.length > 1 && (
                    <div className="instance-choice">
                      <label htmlFor="settings-instance">Local installation</label>
                      <select
                      ...
                      <details className="server-details">
                        ...
                        <div className="settings-row">
                          <div>
                            ...
                            <p>
                              Clear local registration and work. Keep logs and provider
                              configuration.
                            </p>
                          </div>
                          <button
                  ...
                  <section className="settings-section">
                    ...
                    <div className="settings-row">
                      <div>
                        ...
                        <p>
                          Stopping an individual server keeps it stopped on the next app launch.
                        </p>
                      </div>
                      <button
                        type="button"
                  ...
                  <section className="settings-section">
                    <h3>Use the CLI</h3>
                    <p className="body-note">
                      The shared installation uses the same identity and work in the app and
                      terminal. Stop one server before starting the other.
                    </p>
        ...
        {!onboarded && (
          <ServiceStatus
            ...
            name={selected?.name}
            open={() => {
              navigate(onboarded ? "account" : "settings");
              if (!onboarded) setSetupSettings(true);
            }}
          />
        )}
      </main>
